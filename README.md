...
### 1.8.1. Logic
The goal of the Power BI report was to design navigation across the four most important areas our persona focuses on. Each view includes a map on which we marked our properties. In the video presentation we used the Synoptic Panel map, because we wanted close stylistic and color matching. However, since Synoptic Panel requires a paid license, in the Power BI file attached to the documentation we replaced that Synoptic Panel map with Azure Maps. The report contains four views: a Main view, where—besides the map—the key element is the Copilot Smart Narrative summarizing the most important business information; a Reservations view with detailed apartment information; a Cleaning assignments view for assigning cleaners to apartments and tracking in real time which apartments are not yet assigned (thanks to Power Automate, this can be done directly from the report); and a Feedback view that lets you monitor the latest opinions, filter them by apartment, and sets the stage for deeper historical analysis.
